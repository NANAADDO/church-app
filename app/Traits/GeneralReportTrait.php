...
trait GeneralReportTrait
{
    ...
    public function reportLogicProcessor($request, $collectionID, $tempTableName,$colType)
    {
        ...
        $holdeyear=$eyear;
        if($collectionID==5) {
            $member_search_param = ($memberSelectConditon == false ? ($year_select == false ? 'where status_id=? and does_member_want_to_join_welfare =?' : 'where ' . $year_select . " and status_id=? and does_member_want_to_join_welfare =?") : ($year_select == false ? 'where ' : 'where ') . $memberSelectConditon . ' and status_id=? and does_member_want_to_join_welfare =?');
        }
        else{

            $member_search_param = ($memberSelectConditon == false ? ($year_select == false ? 'where status_id=?' : 'where ' . $year_select . " and status_id=?") : ($year_select == false ? 'where ' : 'where ') . $memberSelectConditon . ' and status_id=?');
        }
        $sql_select_members = DB::select($this->fetchMemberDataQuery(). $member_search_param,[1,2]);
 ...
 from ((select pa.new_member_id,cg.name as colname,m.ident,m.name as mname,p.date_paid,p.amount_paid, p.year from months m LEFT JOIN payment_histories p on m.ident = p.month_paid 
   and ".$collectionColumn."  and p.year = ? and p.member_id=? and p.payment_state=? inner join churchgivens cg on ? = cg.id inner join memberdetails pa  on pa.id = ?   " . $month_period . "
    ...
    public function reportLogicYearlyProcessor(Request $request, $collectionID, $tempTableName,$colType)
    {
        ...
        $sql_select_members = DB::select($this->fetchMemberDataQuery() . $member_search_param,[1]);
 ...
 from ((
   ...
   and  c.year=? 
    ...
    $tempname=$tempTableName.$this->getuserid();
        ...
        $db = DB::select($this->fetchMemberDataQuery() . $member_search_param,[1]);
